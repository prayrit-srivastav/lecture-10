...
const getUserCard = function (id) {
  fetch(`https://dummyjson.com/users/${id}`)
    ...
    .then((user) => {
      ...
      if (id - 1 > 0) {
        fetch(`https://dummyjson.com/users/${id - 1}`)
          .then((response) => {
            if (response.status === 404) return;
            return response.json();
          })
          ...
          .catch((error) => {
            console.log("Error fetching previous user:", error);
          });
      } else {
        console.log("No Previous user exists");
      }
      ...
      fetch(`https://dummyjson.com/users/${id + 1}`)
        ...
        .catch((error) => {
          console.log("Error fetching next user:", error);
        });
    })
    .catch((error) => {
